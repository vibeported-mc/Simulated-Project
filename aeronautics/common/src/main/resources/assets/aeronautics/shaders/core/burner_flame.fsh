...
#moj_import <aeronautics:util.glsl>

uniform sampler2D FirePalette;

// 26.2 port: FlameRenderTime and Intensity arrive per vertex now -- see burner_flame.vsh. Palette
// has only two values, so it is baked in as a shader define and the render type is built once per
// palette; a pipeline's uniforms come from engine-filled buffers, so there is nowhere left to set
// one per render type.
